...
def find_repo_root(start: Path) -> Path:
    ...
    raise SystemExit(
        ...
        "Run this script from inside the Daedalus repo."
    )


def read_text(path: Path, label: str) -> str:
    """Read a UTF-8 text file or fail with a useful message."""
    if not path.exists():
...
def choose_config(repo_root: Path, explicit_config: Optional[str]) -> Optional[Path]:
    """Select config file from CLI override or default candidates."""
    if explicit_config:
...
def resolve_workflow(repo_root: Path, workflow_arg: str) -> Path:
    """Resolve workflow alias, relative path, or workflow name."""
...
def build_runner_config(args: argparse.Namespace) -> RunnerConfig:
    """Build runtime config from defaults, config file, and CLI overrides."""
    repo_root = find_repo_root(Path.cwd())
    config_path = choose_config(repo_root, args.config)
    config = load_yaml(config_path)

    ollama_url = args.ollama_url or get_nested(
        config,
        "llm",
        "base_url",
...
def build_prompt_bundle(config: RunnerConfig) -> str:
    """Combine system prompt, workflow contract, and output contract."""
...
def ollama_generate(config: RunnerConfig, prompt: str) -> str:
    ...
    try:
        ...
        raise SystemExit(
            f"ERROR: Ollama returned invalid JSON: {exc}\nBody:\n{body[:500]}"
        ) from exc
...
def write_output(config: RunnerConfig, content: str, workflow_name: str, suffix: str = "md") -> Path:
    """Write generated content to the configured output directory."""
...
def format_output_document(generated: str, config: RunnerConfig) -> str:
    """Add metadata header to model-generated engineering package."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional config path relative to repo root. Default: config/daedalus.local.yml "
            "if present, else config/daedalus.local.example.yml"
        ),
    )
...
def main() -> int:
    """Run Daedalus Phase I package generation."""
    args = parse_args()
    ...
    output_document = format_output_document(generated, config)
